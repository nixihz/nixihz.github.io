---
title : "新手Gopher常犯的50个错误"
description : "阿里云 Composer 全量镜像"
date : "2020-12-22 12:12:02"
categories : 
  -  "golang"
menu : "main"
---
...
无论任何时候，你都不应该将一个控制结构（(if、for、switch或select）的左大括号放在下一行。如果这样做，将会在大括号的前方插入一个分号，否则编译报错。[^semicolon]

[^semicolon]: [陈浩：GO 语言简介（上）— 语法](https://coolshell.cn/articles/8460.html#%E5%85%B3%E4%BA%8E%E5%88%86%E5%8F%B7)
...
```
另一个选择是删除或注释掉未使用的导入：-) goimports工具也可以帮助您。
{{< /tab >}}
{{< /tabs >}}
...
可以使用`vet`命令来查找其中的一些问题。默认情况下，`vet`不执行任何阴影变量检查, 如需检查加上`-shadow`选项  
> `go tool vet -shadow your_file.go`

值得注意的是`vet`不会报出全部的 `shadowed variables`, 使用[`go-nyet`](https://github.com/barakmich/go-nyet) 是一个更好的 `shadowed variables` 检测工具。
...
翻译自[50 Shades of Go: Traps, Gotchas, and Common Mistakes for New Golang Devs](http://devs.cloudimmunity.com/gotchas-and-common-mistakes-in-go-golang/)
